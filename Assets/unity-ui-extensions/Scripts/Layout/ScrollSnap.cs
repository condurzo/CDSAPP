...
namespace UnityEngine.UI.Extensions
{
	...
	public class ScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
	{
		...
		void Update()
		{
			if (this.gameObject.name == "OfertaBanner") {

				if (!AnimacionBotones.activarBanner) {

					if (ScrollSolo.avanzar) {
						NextScreen ();
						//ScrollSolo.avanzar = false;
					}

					if (ScrollSolo.retrazar) {
						PreviousScreen ();
						//ScrollSolo.retrazar = false;
					}
				}
			}
